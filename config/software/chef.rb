...
build do
  ...
  command "find #{install_dir}/embedded/lib/ruby/gems -name '*.rb' -exec grep -l 'File\\.exists?' {} \\; | xargs sed -i 's/File\\.exists?/File.exist?/g'", env: env
end